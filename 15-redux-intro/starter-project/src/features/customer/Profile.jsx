...
export default function Profile() {
  ...
  return (
    <>
    {!modifyn ? <div className='inputs' style={{display:'flex', flexDirection:'column',alignItems:'center',gap:20,marginTop:100}}>
      ...
      <div style={{display:'flex', flexDirection:'row',gap:8}}>
        <button onClick={() => setModifyn(true)}>Modify Details</button>
        <button onClick={handleDelete}>Delete Account</button>
      </div>
    </div>
      :<div>
      <h2>Modify costumer's details</h2>
      <div className="inputs">
        ...
        <button onClick={handleClick} disabled={!(fullName.length > 8 && nationalId.length === 6)}>Modify</button>
      </div>
    </div>}
    </>
  )
